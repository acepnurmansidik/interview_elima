    ...
    public function index(Request $request)
    {
        try {
            ...
            SELECT 
                ...
                CASE when TIME_FORMAT(data_absen.masuk,'%H:%i')  = '00:00' then '-' else TIME_FORMAT(data_absen.masuk ,'%H:%i') end as masuk,
                CASE when TIME_FORMAT(data_absen.pulang,'%H:%i')  = '00:00' then '-' else TIME_FORMAT(data_absen.pulang ,'%H:%i') end as pulang,
                data_absen.hari,
                case when data_absen.hari in (6,7) then 0 when data_absen.hari in (1,2,3,4,5) and data_absen.is_cuti = 0 then 1 when data_absen.hari in (1,2,3,4,5) and data_absen.is_cuti = 1 then 2 WHEN data_absen.terlambat IS NOT NULL THEN 3 else 4 end as status
            FROM 
                m_pegawai mp 
                left join (
                    SELECT 
